library(readxl)
library(ggpubr)
VolcanoData=read_excel("Volcano_GV_MII_YOUNG.xlsx")
colnames(VolcanoData)[1]="label"

VolcanoData$expression = ifelse(VolcanoData$FDRcor <= 0.05 & abs(VolcanoData$log2Foldchange) > 0.58, 
...
p <- ggplot(data = VolcanoData, 
  ...
  geom_text(label=VolcanoData$names1, size=2,colour='black', hjust=0,vjust=0)+
  geom_text(label=VolcanoData$names2, size=2,colour='black', hjust=0,vjust=1)+
  geom_text(label=VolcanoData$names4, size=2,colour='black', hjust=0.2,vjust=-0.2)+
  geom_text(label=VolcanoData$names5, size=2,colour='black', hjust=0.2,vjust=2)+
  geom_text(label=VolcanoData$names6, size=2,colour='black', hjust=0.5,vjust=-0.5)+
  geom_text(label=VolcanoData$names7, size=2,colour='black', hjust=0,vjust=-1)+
